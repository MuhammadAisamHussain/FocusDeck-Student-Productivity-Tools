...
class Store {
    ...
    async getShipments(filters = {}) {
        ...
        if (filters.search) {
            query = query.or(`awb_number.ilike.%${filters.search}%,customer_name.ilike.%${filters.search}%,flight_number.ilike.%${filters.search}%`);
        }
    ...
    async createShipment(shipmentData) {
        ...
        // Trigger task creation for new shipment
    ...
    async updateShipment(id, updates) {
        ...
        // If status changed, trigger task creation
    ...
    async generateAWB() {
        ...
        if (data) return this.generateAWB(); // Recursive retry
        return awb;
    }
    ...
    async getTasks(filters = {}) {
        let query = this.db
            .from('tasks')
            .select('*, shipments(awb_number, customer_name, origin, destination)');
    ...
    async createTaskFromTrigger(trigger, shipment, oldStatus = null) {
        const taskTemplates = this.getTaskTemplates();
        const relevantTemplates = taskTemplates.filter(t => t.trigger === trigger);

        for (const template of relevantTemplates) {
            // Check if this task already exists for this shipment
            ...
            if (!existing) {
                const deadline = this.calculateDeadline(template.deadline_hours);
    ...
    getTaskTemplates() {
        return [
            ...
            },
            {
                ...
                name: 'Send flight details to customer',
                deadline_hours: 2,
                priority: 'high',
                requiredStatus: 'booked'
            },
            {
                trigger: 'status_changed',
                type: 'departure_notice',
                name: 'Send departure confirmation',
                deadline_hours: 1,
                priority: 'high',
                requiredStatus: 'departed'
            },
            {
                trigger: 'status_changed',
                type: 'arrival_notice',
                name: 'Send arrival notice',
                deadline_hours: 1,
                priority: 'high',
                requiredStatus: 'arrived'
            },
            {
                trigger: 'flight_delayed',
            ...
            {
                trigger: 'rate_expiring',
                type: 'renew_rate',
                name: 'Request new rate sheet from airline',
                deadline_hours: 120,
                priority: 'medium'
            }
        ];
    ...
    async getDefaultAssignee(shipment) {
        // For now, assign to a default employee
        // In production, this would use round-robin or workload-based assignment
    ...
    async findBestRate(origin, destination, cargoType, weight) {
        ...
        // Sort by rate and filter valid dates
    ...
    async updateFlightStatus(shipmentId, flightStatus) {
        const statusMap = {
            ...
            'departed': 'departed',
            'arrived': 'arrived',
            'delayed': 'departed' // Keep as departed but trigger delay task
        };
        ...
        if (flightStatus === 'delayed') {
            // Get shipment to trigger delay task
    ...
    async getProfitSummary(period = 'month') {
        ...
        const { data: shipments } = await this.db
            .from('shipments')
            .select('revenue, cost, origin, destination')
    ...
    async getProfitByRoute() {
        const { data } = await this.db
            .from('shipments')
            .select('origin, destination, revenue, cost')
            .not('revenue', 'is', null);
        ...
        data.forEach(s => {
            ...
            if (!routeMap[route]) {
                routeMap[route] = { route, revenue: 0, cost: 0, count: 0 };
            }
